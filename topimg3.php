...
if(isset($_POST["submit"])) {
    ...
    $profile_folder = './static/img/upload/top3.jpg';
    $id = 1;


    $sql = "UPDATE topimg SET img3 = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $profile_folder, $id);

    if ($stmt->execute()) {
        if (move_uploaded_file($profile_tmpname, $profile_folder)) {
            echo "<script>alert(\"Upload Successful\"); window.location='topimg4.php';</script>";
        } else {
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <h1 class="h3 mb-3 fw-normal">Top Pic 3</h1>
