...
if(isset($_POST["submit"])) {
    ...
    $profile_folder = './static/img/upload/top2.jpg';
    $id = 1;


    $sql = "UPDATE topimg SET img2 = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $profile_folder, $id);

    if ($stmt->execute()) {
        if (move_uploaded_file($profile_tmpname, $profile_folder)) {
            echo "<script>alert(\"Upload Successful\"); window.location='topimg3.php';</script>";
        } else {
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <h1 class="h3 mb-3 fw-normal">Top Pick 2</h1>
